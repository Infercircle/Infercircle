...
// Proxy to Helper API for Zerion portfolio
router.get("/portfolio/:address", asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const response = await axios.get(`${process.env.HELPER_APIS_URL}/zerion/portfolio/${address}`);
    res.status(response.status).json(response.data);
  } catch (error: any) {
    if (error.response) {
      res.status(error.response.status).json({
...
// Proxy to Helper API for Zerion positions
router.get("/positions/:address", asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const response = await axios.get(`${process.env.HELPER_APIS_URL}/zerion/positions/${address}`);
    res.status(response.status).json(response.data);
  } catch (error: any) {
    if (error.response) {
      res.status(error.response.status).json({
